...
const tableColumns = [{ accessorKey: 'name', header: 'Name' }, { accessorKey: 'vaultAddress', header: 'Address' }, { accessorKey: 'symbol', header: 'Vault Asset' },  { accessorKey: 'totalAssets', header: 'Total Assets' }]

export const VaultsTable = ({
  vaultsMetadata,
}: VaultsTableProps) => {

  const data = vaultsMetadata?.map((vault) => ({
      ...
      <TableRow
       ...
       key={row.id}>
        {row.getVisibleCells().map((cell) => (
          <TableCell key={cell.id} className="group-hover:text-foreground/90 transition-colors">
